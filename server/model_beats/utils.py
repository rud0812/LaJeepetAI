...
def extract_data(path, midi_files=[]):
    for file_or_dir in os.listdir(path):
        abs_path = os.path.abspathb(os.path.join(path, file_or_dir))
...
def parse_midi(path):
    midi = None
    try:
        midi = pretty_midi.PrettyMIDI(os.path.join(file_prefix, 'content', path))
...
def load_model_from_checkpoint(model_dir):
    ...
    with open(os.path.join(file_prefix, model_dir, 'model.json'), 'r') as f:
        model = model_from_json(f.read())

    epoch = 0
    print(file_prefix + '/' + model_dir)
    newest_checkpoint = max(glob.iglob(file_prefix + '/' + model_dir + '/experiment/checkpoints/*.hdf5'),
